...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			await pb.send(`/api/name-change-requests/${request.pb_id}/reject`, {
				method: 'POST',
				headers: {
					'api-key': key
				}
			...
			if (error.status == 403) return await interaction.editReply(':x: You do not have the permission to reject name change requests');
			else return await interaction.editReply(`:x: Something went wrong while rejecting this request :\n${JSON.stringify(error.response)}`);
		}

		const rejectEmbed = new EmbedBuilder()
			.setColor(0xcc0000)
			.setTitle(':x: Name change Request')
			.addFields(
				{ name: 'Request rejected by', value: `${interaction.user}` },
		...
		await guild.channels.cache.get(requestsArchivesId).send({ embeds: [rejectEmbed] });

		await db.mergeRequests.destroy({ where: { discordid: request.discordid } });

		console.log(`${interaction.user.tag} (${interaction.user.id}) rejected name change request from ${request.user} (${request.user_discord}) to ${request.new_name}`);

		return await interaction.editReply(':white_check_mark: The request has been rejected');
