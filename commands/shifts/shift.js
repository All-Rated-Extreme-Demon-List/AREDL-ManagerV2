...
module.exports = {
	...
	async execute(interaction) {
		...
		if (interaction.options.getSubcommand() === 'add') {
			...
			if (await db.dbShifts.count({ where: { moderator: mod.id } }) == 3) return await interaction.editReply(':x: This moderator already has 3 shifts assigned');

			const addedShift = await db.dbShifts.create({ moderator: mod.id, day: day });
			...
			if (await db.dbShifts.count({ where: { moderator: mod.id, day: day } }) == 0) return await interaction.editReply(':x: This moderator does not have any assigned shift on that day');

			await db.dbShifts.destroy({ where: { moderator: mod.id, day: day } });
			if (await db.dbShifts.count({ where: { moderator: mod.id, day: day } }) == 0) return await interaction.editReply(`:white_check_mark: Successfully removed ${day} shift for ${mod}`);
			else return interaction.editReply(`:x: Couldn't remove ${day} shift for ${mod}, something went wrong`);

		} else if (interaction.options.getSubcommand() === 'list') {

			if (await db.dbShifts.count() === 0) return await interaction.editReply(':x: There are no assigned shifts yet');

			const shifts = await db.dbShifts.findAll({ where: {} });
